...
However, to send messages through carrier waves that is generated by its VCO (Voltage Controlled Oscillator), the newly design RF chip needs the carrier waves to be firmly locked at designated frequencies. An external PLL (Phase-Locked Loop) takes care of this. Without a PLL, the frequency of the carrier waves jitters and messages are lost.

The Raspberry Pi Pico W will send signals to the PLL and control it.

## OVERVIEW
This project has 3 main components:
1. BLE Website (User Interface): The users enter commands for the Pico W here. The commands can be a desired frequency they want the PLL to lock, turning on Pico W LED, or rebooting the Pico W.
2. Raspberry Pi Pico W: Host a BLE server for the website to connect. Listen to the commands from the Website. Directly send serial communication signals (bit banging) to control the PLL.
